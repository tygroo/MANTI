...
import org.hibernate.internal.AbstractSessionImpl.NonContextualJdbcConnectionAccess;
		...
		if (isolationLevelNeeded || definition.isReadOnly()) {
			if (this.prepareConnection) {
				if (definition.isReadOnly()){
					String url = ((DriverManagerDataSource)((DatasourceConnectionProviderImpl)((NonContextualJdbcConnectionAccess)((SessionImpl)session).getJdbcConnectionAccess()).getConnectionProvider()).getDataSource()).getUrl();
					if (url.indexOf(";ACCESS_MODE_DATA=r") == -1)
						((DriverManagerDataSource)((DatasourceConnectionProviderImpl)((NonContextualJdbcConnectionAccess)((SessionImpl)session).getJdbcConnectionAccess()).getConnectionProvider()).getDataSource()).setUrl(url+";ACCESS_MODE_DATA=r");
				}else{
					String url = ((DriverManagerDataSource)((DatasourceConnectionProviderImpl)((NonContextualJdbcConnectionAccess)((SessionImpl)session).getJdbcConnectionAccess()).getConnectionProvider()).getDataSource()).getUrl();
					int position = url.indexOf(";ACCESS_MODE_DATA=r");
					if (position != -1){
						url = url.substring(0,position);
						((DriverManagerDataSource)((DatasourceConnectionProviderImpl)((NonContextualJdbcConnectionAccess)((SessionImpl)session).getJdbcConnectionAccess()).getConnectionProvider()).getDataSource()).setUrl(url);
					}
				}
